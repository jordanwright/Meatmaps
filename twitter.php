<?php
	// Meatmaps config (Engine not required, db config only)
	require_once( dirname(__FILE__) ."/core/config.meatmaps.php" );
	
	// Hybrid config and includes
   	$config = dirname(__FILE__) . '/core/config.hybridauth.php';
    require_once( dirname(__FILE__) ."/core/libs/hybridauth/Hybrid/Auth.php" );
	...
	try{
		...
		$user_profile = $twitter->getUserProfile();
					//Create query
					$pQsessionCheck="SELECT * FROM sessions WHERE session_id='".session_id()."'";
					$eQsessionCheck=mysqli_query($GLOBALS['db'], $pQsessionCheck);
						if(mysqli_num_rows($eQsessionCheck) > 0) {
					...
					//Create query
					$qry="SELECT * FROM member WHERE foreign_id='".$user_profile->identifier."'";
					...
					//Check whether the query was successful or not
						if(mysqli_num_rows($result) > 0) {
							...
							$member = mysqli_fetch_assoc($result);
								...
								mysqli_query($GLOBALS['db'], $pQnewSession) or die(mysqli_error($GLOBALS['db']));
							...
							//make account since not exist
							// username firstname foreign_network foreign_id
							$pQnewSession="INSERT INTO member (`username`, `firstname`, `twitter`, `foreign_network`, `foreign_id`) VALUES ('twt_".$user_profile->identifier."', '".$user_profile->displayName."', '".$user_profile->displayName."', 'twitter', '".$user_profile->identifier."')";
							mysqli_query($GLOBALS['db'], $pQnewSession) or die(mysqli_error($GLOBALS['db']));
							
							//process login
							$pQnewLogin="SELECT * FROM member WHERE foreign_id='".$user_profile->identifier."'";
								$eQnewLogin=mysqli_query($GLOBALS['db'], $pQnewLogin);
							
								//Check whether the query was successful or not
									if(mysqli_num_rows($eQnewLogin) > 0) {
										//Fetch User Deets
										$member = mysqli_fetch_assoc($eQnewLogin);
											$pQnewSession="INSERT INTO sessions (`session_id`, `session_user`) VALUES ('".session_id()."', '".$member['mem_id']."')";
											mysqli_query($GLOBALS['db'], $pQnewSession) or die(mysqli_error($GLOBALS['db']));
										...
										echo "An error has occured. Please contact Jordan Wright.";
									}
						}
